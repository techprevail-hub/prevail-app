// app/dashboard/institute/reports/progressReport/page.tsx
...
export default function ProgressReportPage() {
  ...
  // Sort state
  const [sort, setSort] = useState<{ column: keyof ProgressReportStudent | null; direction: "asc" | "desc" | null }>({
    column: null,
    direction: null
  });
  ...
  const fetchProgressReport = useCallback(async () => {
    try {
      ...
      console.log("Fetched progress report:", response);

      if (response && response.success === true && response.data) {
        // Calculate overall progress for each student
        const studentsWithProgress = response.data.students.map(student => {
          // Calculate overall progress percentage based on completed items
          ...
          const overallProgress = Math.round((completedItems / 4) * 100);
        ...
        setError(
          response?.message || "Failed to fetch progress report."
        );
      }
    } catch (error: any) {
      console.error("Failed to fetch progress report:", error);
      setError(error?.message || "Failed to fetch progress report.");
  ...
  const handleSort = (column: keyof ProgressReportStudent) => {
    setSort((prev) => {
  ...
  const getSortedStudents = (): ProgressReportStudent[] => {
    if (!report) return [];
    ...
    if (sort.column && sort.direction) {
      students.sort((a, b) => {
        let aVal: any = a[sort.column as keyof ProgressReportStudent];
        let bVal: any = b[sort.column as keyof ProgressReportStudent];
  ...
  const columns: { key: keyof ProgressReportStudent | "avatar" | "completion"; label: string; sortable: boolean }[] = [
    { key: "avatar", label: "", sortable: false },
    { key: "name", label: "Student", sortable: true },
    { key: "course", label: "Course", sortable: true },
    { key: "completion", label: "Progress", sortable: false },
    { key: "overallProgress", label: "Overall", sortable: true },
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Track student progress across all performance metrics.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <SummaryCard
          ...
          value={summary.accountActivated}
          icon={User}
          subtitle={`${summary.totalStudents > 0 ? Math.round((summary.accountActivated / summary.totalStudents) * 100) : 0}% completion`}
        />
        <SummaryCard
          title="Resume Completed"
          value={summary.resumeCompleted}
          icon={FileText}
          subtitle={`${summary.totalStudents > 0 ? Math.round((summary.resumeCompleted / summary.totalStudents) * 100) : 0}% completion`}
        />
        <SummaryCard
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm text-slate-500">
                Track individual student progress and completion status.
              </p>
            </div>
                  ...
                  {columns.map(col => (
                    ...
                    >
                      {col.sortable && col.key !== "avatar" ? (
                        <button
                          onClick={() => handleSort(col.key as keyof ProgressReportStudent)}
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        <CompletionBadge 
                          completed={student.accountActivated} 
                          label="Account" 
                        />
                        <CompletionBadge 
                          completed={student.resumeCompleted} 
                          label="Resume" 
                        />
                        <CompletionBadge 
                          completed={student.linkedinCompleted} 
                          label="LinkedIn" 
                        />
                        <CompletionBadge 
                          completed={student.interviewCompleted} 
                          label="Interview" 
                        />
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col">
                        <span className={`font-semibold ${
                          student.overallProgress >= 70 ? 'text-emerald-600' : 
                          student.overallProgress >= 40 ? 'text-amber-600' : 'text-red-600'
                        }`}>
                        ...
                        <div className="w-full bg-slate-100 rounded-full h-1.5 mt-1">
                          <div 
                            className={`h-1.5 rounded-full ${
                              student.overallProgress >= 70 ? 'bg-emerald-500' : 
                              student.overallProgress >= 40 ? 'bg-amber-500' : 'bg-red-500'
                            }`}
                    ...
                    {/* Status */}
                    <td className="px-4 py-3">
                      <ProgressBadge progress={student.overallProgress} />
                    </td>
                  </tr>
                ))}
              </tbody>
